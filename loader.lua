--==================================================
-- OBSIDIAN UI + BEAT SURVIVOR (FIXED TELEPORT)
--==================================================
...
-- Catch Teleport Failures and retry ServerHop automatically
TeleportService.TeleportInitFailed:Connect(function(player, teleportResult, errorMessage)
    if player == Players.LocalPlayer then
        Library:Notify({
            Title = "Teleport Failed",
            Description = "Retrying server hop...",
            Time = 3,
        })
        task.wait(1)
...
local function ServerHop()
    ...
    while Toggles.ServerHop.Value and not Library.Unloaded do
        ...
        local success, result = pcall(function()
            ...
            return HttpService:JSONEncode(game:HttpGet(url))
        end)
        ...
        for _, Server in ipairs(ServersList) do
            ...
            -- FIX: Ubah kriteria ke server yang punya sisa slot aman (tidak langsung penuh/tutup)
            if Server.id
                and Server.id ~= game.JobId
                and Server.playing
                and Server.playing >= 1
                and Server.playing <= (maxPlayers - 2) -- Menghindari server yang hampir penuh/sedang mati
...
AutoFarmGroup:AddToggle("ServerHop", {
    Text = "Server Hop",
    Tooltip = "Hop to low-player servers as Spectator",
    Default = false,
