-- ================================================================
-- MIGRASI DARI OBSIDIAN LIBRARY KE MODERNV2
-- Script: VD Auto Farm (dengan Auto Farm, Webhook, Server Hop)
-- ================================================================

-- Load ModernV2 Library (gunakan raw URL yang sesuai)
local ModernV2 = loadstring(game:HttpGet("https://ziaanclient.vercel.app/zilux"))()

-- Fungsi untuk menonaktifkan error prompt (opsional)
pcall(function()
...
local Window = ModernV2:CreateWindow({
    ...
    Icon = "bot",               -- Icon default (akan di-override)
    Size = ModernV2.Scales.Default,
    -- Tidak ada opsi NotifySide langsung, kita akan atur notifikasi sendiri
    -- ShowCustomCursor tidak tersedia langsung, kita abaikan
})

-- Set sidebar width (tidak ada method langsung, kita bisa set lebar left menu dengan mengubah ukuran)
-- Tapi ModernV2 menggunakan left menu dengan lebar tetap 175, kita bisa ubah dengan mengakses frame.
-- Untuk sederhana, kita biarkan default.
...
-- ================================================================
-- 3. BUAT GROUPBOX (SECTION) DI TAB AUTOFARM
-- ================================================================
-- Left Groupbox (Auto Farm)
local AutoFarmGroup = Tabs.AutoFarm:AddSection({
    Name = "Auto Farm",
    Position = "Left",
    Icon = "zap",
    Box = true,  -- agar terlihat seperti groupbox
})

-- Right Groupbox (Webhook)
...
-- ================================================================
-- 4. BUAT OBJEK UNTUK MENYIMPAN REFERENSI KOMPONEN (seperti Options/Toggles)
-- ================================================================
local Options = {}   -- untuk nilai non-toggle (input, dropdown, slider, keybind)
local Toggles = {}   -- untuk toggle

-- ================================================================
-- 5. FUNGSI NOTIFIKASI (menggunakan Window:Notify)
-- ================================================================
...
local function Notify(title, desc, duration)
    ...
    Window:Notify({
        ...
        Icon = "lucide:bell", -- icon opsional
    })
end

-- ================================================================
-- 6. FUNGSI-FUNGSI UTAMA (sama seperti script asli)
-- ================================================================
...
-- ================================================================
-- 7. WEBHOOK & SNAPSHOT (sama seperti asli)
-- ================================================================
...
-- ================================================================
-- 9. SERVER HOP (dengan ignore list)
-- ================================================================
...
-- ================================================================
-- 10. TAMBAH KOMPONEN UI KE GROUPBOX
-- ================================================================
...
-- Tooltip tidak langsung, bisa ditambahkan dengan metode ToolTip?
-- ModernV2 tidak punya Tooltip bawaan, tapi kita bisa abaikan.
...
-- Toggle untuk membuka keybind menu (tidak ada keybind frame di ModernV2, kita abaikan atau buat custom)
-- Kita bisa gunakan toggle untuk menampilkan keybind list? Tidak ada, jadi kita lewati.
-- Atau kita bisa buat toggle untuk menampilkan/menyembunyikan window? Sudah ada keybind.

-- Custom Cursor tidak didukung langsung, kita abaikan.

-- Notification Side: ModernV2 tidak punya set side, notifikasi selalu di kanan atas? Kita bisa atur dengan mengubah posisi notifikasi? Tidak mudah. Kita abaikan.

-- DPI Scale: ModernV2 tidak punya DPI scaling, kita abaikan.

-- Corner Radius: ModernV2 punya Window:SetCornerRadius? Tidak, tapi kita bisa akses UICorner dari Window.Root. Kita buat fungsi.

MenuGroup:AddSlider({
    Name = "Corner Radius",
    Flag = "UICornerSlider",
    Default = 10, -- nilai default
    Min = 0,
    Max = 20,
    Rounding = 0,
    Callback = function(v)
...
Options.MenuKeybind = MenuGroup:AddKeybind({
    ...
    Callback = function(state)
        -- Toggle window saat keybind ditekan
        if state then
...
-- ================================================================
-- 12. THEME & CONFIG (menggunakan bawaan ModernV2)
-- ================================================================
-- ModernV2 sudah memiliki sistem theme dan config.
-- Kita bisa menambahkan tema default (opsional)
ModernV2:AddTheme({
    Name = "Default",
...
-- Konfigurasi otomatis akan menyimpan semua flag yang terdaftar.
-- Kita atur folder config (default sudah ada)
-- Kita bisa set config folder dengan Window.ConfigFolder (sudah diatur saat CreateWindow)

-- Agar config otomatis load saat start, kita set ConfigAutoLoad = true (default true)
-- Untuk auto-save, kita set ConfigAutoSave = true (default true)
-- Tidak perlu konfigurasi tambahan.

-- Untuk mengabaikan beberapa flag dari save (seperti MenuKeybind), kita bisa
-- menggunakan Window.ConfigManager:IgnoreIndexes? Tidak ada method, tapi kita bisa
-- menghapus flag dari daftar? Lebih mudah: kita tidak daftarkan flag untuk MenuKeybind,
-- tapi kita sudah daftarkan. Karena ModernV2 akan menyimpan semua flag, kita biarkan saja.

-- Kita jalankan QueueAutoExec sekali
QueueAutoExec()

-- ================================================================
-- 13. LOOP UTAMA (BeatGame)
-- ================================================================
task.spawn(function()
...
-- ================================================================
-- 14. (Opsional) Aktifkan window
-- ================================================================
Window.Signal:SetValue(true) -- tampilkan window
